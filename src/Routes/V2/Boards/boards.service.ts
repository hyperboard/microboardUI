...
export class BoardsService {
  ...
  async validateAuthor(boardUUID: string, authorUUID: string) {
    const [board] = await this.db
      ...
      .where(and(
        ...
        eq(boards.authorUUID, authorUUID),
      ))
  ...
  async claim(authorKeys: string[], ownerId: number) {
    await this.db.transaction(async (tx) => {
      const selectQuery = tx
        ...
        .where(
          and(
            eq(boards.authorUUID, sql.placeholder('authorKey')),
            isNull(boardOwner.ownerId)
          )
        )
        .prepare('selectBoardByAuthorKey');

      const boardIds = await Promise.all(authorKeys.map(
        async (key) => {
    ...
    if (boardUUIDs.length > 0) {
      await this.db.transaction(async (tx) => {
        const [sharedFolder] = await tx
          ...
          .from(folders)
          .where(and(
            eq(folders.type, FolderType.VISITED),
        ...
        const sharedFolderItems = await tx
          ...
          .where(and(
            eq(foldersToBoards.folderId, sharedFolder.id),
          ))
        ...
        const boardIds = await Promise.all(boardUUIDs.map(async (uuid) => {
          const [board] = await tx.select({ id: boards.id, uniqId: boards.uniqId })
            .from(boards)
            .where(eq(boards.uniqId, uuid))
          return board
        }))
        await Promise.all(boardIds.map(async (board) => {
  ...
  async getAllBoardLastEventOrders() {
    const extractedValuesQuery = this.db
      .$with('extracted_values')
      .as(
        this.db
          .select({
            boardId: boardEvents.boardId,
            eventId: boardEvents.eventId,
            integer2Value: sql`CAST((regexp_matches(${boardEvents.eventId}, '(\\d+):(\\d+)'))[2] AS INTEGER)`.as('integer2Value')
          })
          .from(boardEvents)
      );

    const lastOrderPerBoardQuery = this.db
      .$with('last_order_per_board')
      .as(
        this.db
          .select({
    ...
    const results = await this.db.with(extractedValuesQuery, lastOrderPerBoardQuery)
      .select({
  ...
  async getEventsCountSinceLastSnapshot(boardUUID: string) {
    ...
    const eventCountsCTE = this.db
      .$with('event_counts')
      .as(
        this.db
          .select({
            last_snapshot_order: sql`COALESCE((SELECT ${lastSnapshotCTE.last_event_order} FROM ${lastSnapshotCTE}), -1)`.as('last_snapshot_order'),
            total_events: sql`COUNT(*)`.as('total_events'),
            events_since_snapshot: sql`COUNT(CASE WHEN ${parsedEventsCTE.event_order} > COALESCE((SELECT ${lastSnapshotCTE.last_event_order} FROM ${lastSnapshotCTE}), -1) THEN 1 END)`.as('events_since_snapshot')
          })
  ...
  async getGrantedUsers(boardId: number) {
    ...
    const owner = await this.db
      .select({
        ...getTableColumns(users), name: userNames.name,
        avatar: userAvatars.avatar,
