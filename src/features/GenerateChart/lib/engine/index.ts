...
export class LayoutEngine {
	...
	protected transformConnector(options: {
		...
		const startRelative = toRelativePoint(
			...
			if (!visited.has(node.id)) {
				if (!this.rows[lastRow]) {
					this.rows[lastRow] = {
						...
						y:
							lastRow *
							(this.DEFAULT_NODE_HEIGHT + this.VERTICAL_GAP),
					};
				}
	...
	private getShapePosition(
		...
		const x =
			...
			metrics.currentIndex *
				(this.DEFAULT_NODE_WIDTH + this.HORIZONTAL_GAP) +
			...
			if (object.itemType === "node") {
				...
				this.transformShape({
					...
					rowInfo: {
						...
						totalInRow:
							rowMetrics.get(
								this.nodeLayouts.get(object.id)?.row ?? 0,
							)?.totalItems ?? 1,
						indexInRow:
							this.nodeLayouts.get(object.id)?.column ?? 0,
					},
				});
			}
		});
				...
				if (nodeMatch) {
					...
					if (!declaredNodes.has(nodeName)) {
						...
						objects.push({
							...
							itemType: "node",
							label:
								attrs.label?.replace(/^"(.*)"$/, "$1") ||
								nodeName,
							shape:
								attrs.shape?.replace(/^"(.*)"$/, "$1") ||
								"rectangle",
						});
					}
				}
