...
class PublicBoardsBase extends React.PureComponent<Props, PublicBoardsState> {
	...
	handleDrop = (
		...
		if (draggedBoardId && dragOverBoardId) {
			...
			this.props.app.storage.reorderPublicBoard(
				draggedBoardId,
				dragOverBoardId,
			);
			this.setState({
				...
				additionalAction={{
					...
					action: () => {
						...
						app.createPublicBoard().then((id: stirng) => {
							app.openBoard(id);
							this.props.router.navigate(`/boards/${id}`, {
								replace: true,
							});
						});
					},
			...
			>
				{boards.map(board => (
					<div
						...
						draggable
						onDragStart={e =>
							this.handleDragStart(e, board.boardId)
						}
					...
					>
						<Menu
							...
							heading={
								board.name
									? board.name
									: `${board.boardId.substring(
											0,
											5,
									  )}...${board.boardId.substring(
											board.boardId.length - 5,
									  )}`
							}
							offset={offset}
							onClick={() => {
							...
							isRenaming={
								this.state.renamingItem === board.boardId
							}
							onContextMenu={(x, y) => {
							...
							onRename={newName => {
								this.handleSave(board.boardId, newName);
								this.handleRename();
							}}
						></Menu>
					</div>
