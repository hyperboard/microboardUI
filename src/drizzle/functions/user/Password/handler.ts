...
export async function addPassword(userId: number, password: string) {
    await db.insert(userPasswords).values({ userId, password }).execute();
}
...
export async function getPassword(userId: number) {
    const [passwordRecords] = await db
        .select({ password: userPasswords.password })
        .from(userPasswords)
...
export async function checkPassword(userId: number, password: string) {
    const passwordRecords = await db
        .select()
        .from(userPasswords)
        .where(and(eq(userPasswords.userId, userId), eq(userPasswords.password, password)))
        .execute();
...
export async function deletePassword(userId: number) {
    await db.delete(userPasswords).where(eq(userPasswords.userId, userId)).execute();
}
